...
async def connect():
    ...
    dial_options = DialOptions.with_api_key(
        os.getenv("API_KEY"), os.getenv("API_KEY_ID")
    )
...
async def main():
    ...
    for i in range(number):
        robot_ids.append(
            await app_client.new_robot(name + "_" + str(i), os.getenv("LOCATION_ID"))
        )
    #
    for i in robot_ids:
        ...
        f.write(
            '{"cloud":{"app_address":"https://app.viam.com:443","id":"'
            + robot_parts[0].id
            + '","secret":"'
            + robot_parts[0].secret
            + '"}}'
        )
        f.close()
